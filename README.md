...
- Normal 模式始终保持英文；Visual、Command 等其他 normal-like 模式进入时默认英文，之后允许手动切换。
- Insert 模式根据光标两侧内容自动选择中文、大写或英文。
- 光标两侧同时匹配时优先使用左侧结果。
- 所有匹配都会忽略数字和空格，它们本身不影响匹配结果。
- 英文匹配识别英文字母和英文半角标点；大写匹配只额外跳过 `-`、`_`。
- Insert 模式通过光标颜色显示中文、英文和 CapsLock 状态；normal-like 模式恢复编辑器原生光标颜色。
- 切换前校验编辑器焦点和 Windows 前台窗口，避免排队事件影响外部程序。
- 只关闭插件自己开启的 CapsLock，不修改用户手动开启的 CapsLock。
...
| 场景 | 输入法行为 | 光标颜色 |
| --- | --- | --- |
| Normal 模式 | 始终保持英文 | 编辑器原生颜色 |
| Visual、Command 等其他 normal-like 模式 | 进入或重新聚焦时默认英文，随后允许手动切换 | 编辑器原生颜色 |
