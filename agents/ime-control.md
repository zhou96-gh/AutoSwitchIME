...
精确 Normal 模式必须始终保持英文；IntelliJ 在任意按键释放后延迟回读 Rime 状态，检测到中文时通过 Coordinator 重新提交英文动作，不绑定具体输入法快捷键。两端状态文件监听即使已启用文件系统事件，也必须每 500ms 主动回读作为漏事件兜底。所有纠正动作继续执行请求有效性、编辑器焦点和前台窗口校验。Visual、Command 等其他 normal-like 模式只在进入或重新聚焦时默认英文，默认动作成功后允许用户手动切换。所有 normal-like 模式都恢复 IDE 原生光标颜色。
